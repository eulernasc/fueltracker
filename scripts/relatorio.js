...
// ── Busca dados do Firestore via REST ─────────────────────────
async function firestoreGetAll(collection) {
  ...
  do {
    ...
    const pageDocs = (data.documents || []).map(doc => parseDoc(doc));
...
async function gerarRelatorio() {
  ...
  for (const grupo of GRUPOS) {
    ...
    try {
      ...
      console.log(`📧 E-mail enviado: ${grupo.nome} → ${grupo.destinatarios.join(', ')}`);
    } catch (err) {
      console.error(`❌ Erro ao enviar ${grupo.nome}:`, err.message);
    }
  }
...
<body style="margin:0;padding:0;background:#f3f4f6;font-family:'Segoe UI',Arial,sans-serif">
  <div style="max-width:680px;margin:32px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.08)">
    ...
    <div style="background:#fef3c7;border-left:4px solid #f59e0b;padding:16px 32px">
      <div style="font-weight:700;color:#92400e">⚠️ ${maquinas.length} máquina${maquinas.length>1?'s':''} da operação <strong>${nomeGrupo}</strong> abaixo de ${limite}% de eficiência</div>
      <div style="font-size:13px;color:#b45309;margin-top:4px">Período: últimos ${diasJanela} dias</div>
    </div>
